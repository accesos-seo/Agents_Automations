...
/**
 * Hook maestro: dado un analysisRequestId, suscribe al cambio de TODAS las
 * tablas relevantes del pipeline y devuelve el estado derivado + el array
 * de stages listo para pintar el checklist.
 */
export function useAnalysisPipeline(analysisRequestId: string | undefined) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      // Derivar run_ids desde response_payload.ingestion_results cuando exista
      const runIdsFromPayload = extractRunIds(request);
      const clientId = extractClientId(request);

      // 2. Cargar tablas dependientes en paralelo
      const [runsRes, soRes, cmpRes, diagRes, findRes, recRes, repRes] = await Promise.all([
        runIdsFromPayload.length
          ? supabase.schema(SCHEMA).from('analysis_runs').select('*').in('id', runIdsFromPayload)
          : Promise.resolve({ data: [] as AnalysisRun[], error: null }),
        clientId
          ? supabase
              .schema(SCHEMA)
              .from('site_overview')
      ...
      setState({
        request,
        runs: (runsRes.data as AnalysisRun[]) ?? [],
      ...
      const reqChannel = supabase
        ...
        .on(
          'postgres_changes',
          {
            ...
            filter: `id=eq.${analysisRequestId}`,
          },
          (payload) => {
            const next = payload.new as AnalysisRequest;
            setState((s) => ({ ...s, request: next }));
          },
        )
        .subscribe();
      unsubs.push(() => supabase.removeChannel(reqChannel));
        ...
        const clientChannel = supabase
          ...
          .on(
            ...
            (p) =>
              setState((s) => ({
                ...s,
                findings: [...s.findings, p.new as AgentFinding].sort(
          ...
          .subscribe();
        unsubs.push(() => supabase.removeChannel(clientChannel));
      }
    }
  ...
  const isComplete = state.request?.request_status === 'complete' && state.report?.report_status === 'generated';
  const hasError =
    state.request?.request_status === 'error' || stages.some((s) => s.state === 'error');
...
function extractRunIds(req: AnalysisRequest): string[] {
  const ingestion =
    (req.response_payload as { ingestion_results?: Array<{ run_id?: string }> } | null)?.ingestion_results ??
    [];
  return ingestion.map((r) => r.run_id).filter((x): x is string => Boolean(x));
}
...
function deriveStages(s: PipelineState): Stage[] {
  ...
  const stages: Stage[] = [
    {
      key: 'submitted',
      label: 'URL recibida y validada',
      state: r ? 'done' : 'pending',
      detail: r?.target_url,
    },
    {
      key: 'queued',
      label: 'Solicitud encolada',
      state: status
        ? ['ready_for_dispatch', 'queued'].includes(status)
          ? 'running'
          : 'done'
        : 'pending',
    },
    {
      key: 'dispatched',
      label: 'Despachada al orquestador',
      state: !status
        ? 'pending'
        : status === 'error'
          ? 'error'
    ...
    {
      key: 'ingest_keywords',
      label: 'Ingesta de keywords orgánicas',
      state: ingestionDone('organic_keywords') ? 'done' : orchestrationActive ? 'running' : 'pending',
    },
    {
      key: 'ingest_top_pages',
      label: 'Ingesta de Top Pages',
      state: ingestionDone('top_pages') ? 'done' : orchestrationActive ? 'running' : 'pending',
    },
    {
      key: 'ingest_backlinks',
      label: 'Ingesta de Backlinks',
      state: ingestionDone('backlinks') ? 'done' : orchestrationActive ? 'running' : 'pending',
    },
    {
      key: 'ingest_referring_domains',
      label: 'Ingesta de Referring Domains',
      state: ingestionDone('referring_domains') ? 'done' : orchestrationActive ? 'running' : 'pending',
    },
    {
      key: 'site_overview',
      label: 'Snapshot del dominio (DR, tráfico, valor)',
      state: s.siteOverview ? 'done' : orchestrationActive ? 'running' : 'pending',
      detail: s.siteOverview
        ? `DR ${s.siteOverview.domain_rating ?? '—'} · ${formatNumber(s.siteOverview.organic_traffic)} visitas/mes`
        : undefined,
    },
    {
      key: 'comparison',
      label: 'Comparativa histórica vs snapshot previo',
      state:
        s.comparison?.comparison_status === 'complete'
          ? 'done'
          : s.comparison?.comparison_status === 'error'
            ? 'error'
            : orchestrationActive
              ? 'running'
              : 'pending',
    ...
    {
      key: 'diagnostic',
      label: 'Diagnóstico estratégico (causas + hallazgos)',
      state:
        s.diagnostic?.diagnosis_status === 'complete' || s.findings.length > 0
          ? 'done'
          : orchestrationActive
            ? 'running'
            : 'pending',
    ...
    {
      key: 'report',
      label: 'Informe final generado',
      state: s.report?.report_status === 'generated' ? 'done' : orchestrationActive ? 'running' : 'pending',
    },
  ];
