  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: rep, error: repErr } = await supabase
        .schema(SCHEMA).from('reports').select('*')
      ...
      if (rep) {
        const { data: secs } = await supabase
          .schema(SCHEMA).from('report_sections').select('*')
          .eq('report_id', (rep as Report).id)
          .order('section_order', { ascending: true });
        if (!cancelled) setSections((secs as ReportSection[]) ?? []);

        const ch = supabase
          .channel(`report:${(rep as Report).id}`)
          .on('postgres_changes',
            { event: '*', schema: SCHEMA, table: 'report_sections', filter: `report_id=eq.${(rep as Report).id}` },
            (payload) => {
              setSections((prev) => {
