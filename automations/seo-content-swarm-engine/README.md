...
| D-009 | 2026-05-17 | **Audio v15: migración a Gemini TTS via OpenRouter** | Modelo `google/gemini-3.1-flash-tts-preview` vía `OPENROUTER_API_KEY` existente. TTS puro = determinista por arquitectura (el modelo solo convierte texto a voz, no razona). Endpoint `/v1/audio/speech`. Voces: Schedar/Sulafat (es-MX), Charon/Aoede (pt-BR), Kore (en-US). Añade columnas `audio_script` (guion editable) y `prompt_version` a `content_audio_items`. Patch completo en `propuestas-ops-control-plane/06`. |
...
| 2026-05-17 | D-009 — Audio v15: migración a `google/gemini-3.1-flash-tts-preview` via OpenRouter. Determinismo por arquitectura. Patch completo en `propuestas-ops-control-plane/06`. |
