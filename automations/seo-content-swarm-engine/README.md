...
| D-010 | 2026-05-16 | **Email de confirmación al cliente cuando envía feedback** (→ migrada a automatización propia [`client-feedback-notifications`](../client-feedback-notifications/)) | `submit-client-article-feedback` v3 en Light_House. Tras INSERT en `client_article_feedback`, llama Mailjet directamente. **Idioma resuelto desde el cliente** (no del artículo): `cliente_users.language` → `clientes.language` → `"es"` default. 3 plantillas (es/pt/en) × 4 variantes por `tipo` (aprobacion verde, rechazo rojo, mejora ámbar, correccion índigo). Incluye `feedback_id` como referencia. Envío no-fatal: si Mailjet falla, el feedback queda guardado y el error se loguea sin romper la respuesta. **Pendiente:** setear `clientes.language` para Doug Construction (`en`), Armor Corp, Holisteek, Vozy.ai (hoy NULL → usan default `es`). |
...
| 2026-05-16 | D-010 — Email de confirmación a cliente tras feedback. `submit-client-article-feedback` v3 deployado. Idioma desde `cliente_users.language` → `clientes.language` → `"es"`. Trilingüe (es/pt/en), 4 variantes por tipo, no-fatal. **Migrado a automatización propia** [`client-feedback-notifications/`](../client-feedback-notifications/). |
