...
| D-010 | 2026-05-16 | **Email de confirmación al cliente cuando envía feedback** | `submit-client-article-feedback` actualizado a v2 en Supabase Light_House. Tras el INSERT en `client_article_feedback`, llama a Mailjet directamente (credenciales `MAILJET_API_KEY`/`MAILJET_SECRET_KEY` ya disponibles en el proyecto). Email bilingüe: pt-BR si `content_items.language` empieza con "pt", español en cualquier otro caso. 4 variantes por `tipo`: aprobacion (verde), rechazo (rojo), mejora (ámbar), correccion (índigo). Incluye referencia del feedback_id. Envío no-fatal: si falla Mailjet, el feedback queda guardado y el error se loguea sin romper la respuesta. |
...
| 2026-05-16 | D-010 — Email de confirmación a cliente tras feedback. `submit-client-article-feedback` v2 deployado. Mailjet directo, bilingüe (pt-BR/es), 4 variantes por tipo, no-fatal. |
