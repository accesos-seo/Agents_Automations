...
**Archivos / componentes que tocas:**
- Edge Function `seo-content-orchestrator` (v4.4 / Supabase v43, productiva)
...
**Decisiones tomadas que aplican aquí:** D-002 (política de competidores — capa 6), D-003 (los 7 artículos reescritos manualmente como referencia de calidad), D-009 (prompts mejorados: dato en apertura + marca en CTA).
...
**Archivos / componentes que tocas:**
- Edge Function `seo-content-quality-enforcer` (**v1.0, ACTIVA** — deployada D-009)
...
- Tabla `content_generation_logs` (filtros por step `quality_enforcer`)
- Tabla `content_generation_alerts` (124 abiertas hoy)

**Decisiones tomadas que aplican aquí:** D-002 (política de competidores — capa 7), D-009 (Quality Enforcer deployado).

**Estado del Quality Enforcer (D-009):**

✅ `seo-content-quality-enforcer` v1.0 está en producción. Checks activos:

| Check | Tipo | Aplica a |
|---|---|---|
| Competidores iGaming pt-BR (16) | 🔴 Bloqueante | cassino-bet, vera-bet |
| Dato cuantificable en primera oración | 🔴 Bloqueante | todas las marcas |
| Nombre de marca en CTA | 🔴 Bloqueante | todas las marcas |
| Producto propio mencionado | 🟡 Warning | todas las marcas |
| FAQ con respuestas sustanciales | 🟡 Warning | todas las marcas |

**Pendientes en esta área:**
- **Monitorear** tasa de bloqueo en producción (ver query en propuesta 07).
- **Ajustar umbrales** del Quality Enforcer si tasa de falsos positivos es alta.
- Limpiar las 124 alertas abiertas: bulk close para causas ya resueltas + 1 a 1 para casos complejos.
- Añadir competidores para marcas restantes (armor-corp, doug-construction, etc.) cuando se definan.
- Activar `seo-content-contract-validator-agent` (v3.1) en marca piloto cuando se active swarm v3.1.
...
- **[D-007 pendiente]** Arquitectura 4 fases propuesta en propuesta 06. Pendiente confirmación P-1 a P-4 del usuario.
...
**Pendientes en esta área:**
- Aplicar los patches 01-07 en `ops-control-plane` (lista para que el desarrollador lo haga).
...
1. ¿Hay algo crítico bloqueando producción? → **Imágenes (E)** o **Validator (D)** monitoreo de quality enforcer.
