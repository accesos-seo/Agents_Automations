...
| `notifications_outbox` | Bandeja de salida. Esta automatización escribe con `source = 'client_requests_attention'`, `type = 'client_requests_attention'` | 🟢 |
| `users` | Resuelve `slack_id` del especialista asignado para DM de nivel 1 | 🟢 |
| `business_calendar` | Calendario laboral Bogotá — no integrado aún en la lógica del SP | 🟡 existe, sin uso en este flujo |
...
| Vista | Rol | Estado |
|---|---|---|
| `client_requests_attention` | Filtro + categorización de solicitudes activas. Calcula `attention_reason`, `days_overdue`, `days_waiting` | 🟢 |

---

## Edge Functions

| Función | Slug | Versión | `verify_jwt` | Estado |
|---|---|---|---|---|
| `client-requests-attention-runner` | `client-requests-attention-runner` | v38 | `false` | 🟢 activa |
...
|---|---|---|
| `CLIENT_REQUESTS_CHECK_SECRET` | Autenticación del endpoint (header `x-internal-secret`) | Configurado |
| `SLACK_POST_MESSAGE_ENDPOINT` | URL de la API de Slack para enviar mensajes | Requerido |
...
| `C09SN85SGKC` | Nivel 2 | Canal privado de supervisores | 🔴 bot sin acceso — notificaciones fallan |
...
|---|---|---|
| 2026-05-04 | EF v1–v37 | Construcción inicial y ajustes iterativos |
| 2026-05-05 | EF v38 | Versión estable. Primera ejecución exitosa |
| 2026-05-17 | SP `run_client_requests_attention_check` | Corrección D-002: re-alerta en nivel 3 cada 2 días. La EF no cambió. |
