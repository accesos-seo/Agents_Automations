...
function clean(h:string){return h.replace(/<script(?![\s\S]*?data-cab)(?![\s\S]*?data-fz)[\s\S]*?<\/script>/gi,"").replace(/(<(?!script)[^>]+?)\s+on[a-z]+\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi,"$1").replace(new RegExp("(?<!['\"])javascript\\s*:","gi"),"");}
